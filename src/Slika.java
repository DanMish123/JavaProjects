...
public class Slika {
    // Podpisi tipov slikovnih datotek
    ...
    public Slika(String pot, String tip, int s, int v) {
        this.pot = pot;
        this.tip = tip;
        this.sirina = s;
        this.visina = v;
    ...
    // Preveri, če datoteka ustreza podpisu (tabela bajtov)
    private static boolean preveriPodpis(String vhodnaDatoteka, int[] podpis) {
        try (DataInputStream dis = new DataInputStream(new FileInputStream(vhodnaDatoteka))) {
            ...
            System.out.printf("Napaka: pri branju datoteke %s.\n", vhodnaDatoteka);
        }
        return false;
    }
    ...
    public static Slika preberiPNG(String vhodnaDatoteka) {
        try (DataInputStream dis = new DataInputStream(new FileInputStream(vhodnaDatoteka))) {
            // preskočimo podpis (8 B) ter dolžino (4 B) in tip (4 B) kosa IHDR
            dis.skipBytes(8 + 4 + 4);
            // readInt() prebere 4 bajte podatkov in jih interpretira kot int
